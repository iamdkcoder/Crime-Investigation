...
from tensorflow.keras.applications.efficientnet import EfficientNetB2 , preprocess_input as preprocessE
from tensorflow.keras.applications.densenet import DenseNet169,preprocess_input as preprocessD
...
class EfficientNetFeatures:
    ...
    def extract(self, img):
        ...
        x = preprocessE(x)  # Subtracting avg values for each pixel
...
class DenseNetFeatures:
    ...
    def extract(self, img):
        ...
        x = image.img_to_array(img)  # To np.array. Height x Width x Channel. dtype=float32
        x = np.expand_dims(x, axis=0)  # (H, W, C)->(1, H, W, C), where the first elem is the number of img
        x = preprocessD(x)  # Subtracting avg values for each pixel
        feature = self.model.predict(x)[0]  # (1, 4096) -> (4096, )
        return feature / np.linalg.norm(feature)  # Normalize
